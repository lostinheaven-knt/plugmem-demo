...
def test_retrieve_structured_filters_invalid_citations(tmp_path: Path) -> None:
    ...
    ans = plugmem.retrieve_structured(query="What should I do?")

    # FakeLLM returns no citations; filter should still mark metadata.
    assert ans.metadata.get("citations_filtered") is True
    assert isinstance(ans.cited_items, list)
